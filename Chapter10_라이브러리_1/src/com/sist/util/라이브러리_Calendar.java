 ...
 *            hasMoreTokens() : 분리된 갯수만큼 반복문(루프) 수행
 ...
 *             => 요청 ========> 요청처리(응답)  
 *        => 기본형 / Wrapper는 호환
 *           Integer i= 10; ==> 실제값(리터럴) 대입  //오토박싱(박싱)
 ...
 *                    날짜 설정
 *              set() : 년/월/일/시간/분/초
 *              get() : 요일 읽기 => get(Calendar.DAY_OF_WEEK)
 *                      각달의 마지막 날 => getActualMaximum()
 *        = 배열을 대체
 *          3대 클래스 : String, Integer, List
...
public class 라이브러리_Calendar {
	...
	public static void main(String[] args) {
		...
		// 추상클래스는 미원성된 클래스 (메소드가 구현이 안된 상태)
		// 구현한 클래스를 이용한다거나, 싱글턴을 이용한다.
		// 싱글턴 => newInstance(),getInstance()
		/*
		 *    static => 클래스명.메소드명()
		 *              Math.random()
		 *    클래스명 객체명 = new 생성자()
		 *    ------            ------ 같으면, 일반클래스
		 *                             다르면, 추상클래스 / 상속 /인터페이스 => 클래스명 객체명 = new 메소드();
		 *                                                                                 ------
		 *                                                                                        
		 */
		// 1일자의 요일을 가지고 온다. (4 => 4월 1일)
		cal.set(Calendar.YEAR,year);
		cal.set(Calendar.MONTH, month-1); //입력(설정) 시 -1
		cal.set(Calendar.DATE, 1);
		
		// 요일 구하기
		int week = cal.get(Calendar.DAY_OF_WEEK)-1;//week은 1부터 시작
		...
		{
			if(i==1)// 맨 처음 공백 : 1일이 시작되는 요일에 출력
			{

...
//	======================================= 클래스화
	...
	public List<EmpVO> empListData()
	{
		...
		try
		{
			...
			// 4. 오라클에 명령을 내린다. (SQL 문장 실행후 결과값을 가지고 온다.)
			 ...
			 *     7789       -       -       -          -
			 *    -------------------------------------------
			 *     getInt getString getString getDate getDouble ==> 위치(rs.previous())
			 *    -------------------------------------------
			 *    
			 */
			while(rs.next())
	...
	// 3. 검색
	public List<EmpVO> empFindEname(String alpha)
	{
		List<EmpVO> list = new ArrayList<EmpVO>();
		try
		{
			getConnection();
			String sql ="SELECT ename,empno,job,deptno FROM emp WHERE ename LIKE UPPER('%"+alpha.toUpperCase()+"%') ORDER BY 1";
			ps=conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while(rs.next())
			{

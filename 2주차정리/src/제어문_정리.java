 ...
 *            2. int이하는 연산시에 결과는 int
 *            3. 모든 연산은 같은 데이터형끼리 할 수 있다. (자동 형변환)
 *            4. 비교연산자는 기본형만 가능하다.
 *               boolean은 ==, !=만 사용이 가능
 *               분자열은 비교연산자를 사용하지 않는다.
 *            5. &&, ||는 효율적인 연산을 한다.
 *               += ==> int a=10;
 *                      a+=10 ==> a=a+10
 *                  
 ...
 *    --------------------------------------------------- + Component(class)
 *    데이터만 묶은 클래스 : ~VO(Value Object)
 *    기능만 묶는 클래스 : ~DAO(Data Access Object), ~Manager, ~Service

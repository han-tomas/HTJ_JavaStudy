 ...
 *         = 다른 데이터형 : 클래스(***)
 *    3. 클래스
 *        = 클래스 정의 방법(객체 지향 프로그램 => 한개의 클래스로 만드는것이 아니라 
 ...
 *                    => 윈도우, Thread(프로그램 안에서 여러개의 프로그램을 수행)
 *          protected : 같은 패키지 않에서만 사용이 가능
 ...
 *              공유 변수(static) : 패턴(싱글턴) => 한개의 객체만 사용
 *                     ------ 메모리 공간이 한개  
 *                     ------ 공유하는 데이터가 존재                    
 *             -----------------------------------------------------------------------
 *              생성자 
 *                1. 클래스명과 동일
 *                2. 멤버변수에 대한 초기화
 *                3. 생성자가 없는 경우에는 자동으로 컴파일러가 기본 생성자를 추가
 *                                                 -------- 매개변수가 없는 생성자
 *                   예)
 *                      class A
 *                      {
 ...
 *                   사용법) new 생성자()
 *                6. 리턴형은 존재하지 않는다.
 *                7. 생성자는 필요시에는 여러개 만들 수 있다.                            기술면접
 *                   -----------------------------                           ---------------------
 *                          오버로딩을 지원한다(중복메소드 정의)                        ***오버로딩 vs 오버라이딩?
 *                          ----- 한개의 메소드 이름으로 여러개 기능을 생성                  new      modify
 *                          조건) 메소드명이 동일                                      은닉화 캡슐화?
 *                               한개의 클래스에서 만든다                               객체지향에서 객체란?
 *                               매개변수의 갯수나 데이터형이 다르다                    ---------------------
 *                               리턴형은 관계가 없다.
 *                           예)
 *                              class A
 *                              {
 *                                 //A(){} 생성자가 있는경우 기본생성자는 추가되지 않는다.
 *                                 A(int a){}
 *                                 A(double d){}
 *                                 A(int a, int b){}
 *                              }
 ...
 *                              {
 *                              }
 *                              A a = new A(); // 생성자가 없기 때문에 기본생성자 A(){}가 생성되고
 *                                                따라서, 오류 발생X
 *                         
 *                          *** 생성자는 초기화 담당
 ...
 *                    
 *               매개변수 : 여러개 사용이 가능, 사용자가 요청한 값.   
 *                       ------------- 매개변수는 최소화를 시킨다
 *                                     매개변수 3개 이상이면 배열, 클래스를 이용한다.
 *               구현부 : 변수가 선언이 되면 => 지역변수 (메소드 {} 종료시에 사라진다.)
 *                      지역변수 : 초기화가 안되어있다. (반드시 초기화 후에 사용해야 한다.) 
 *           
 *           *** 변수/메소드                                                                    
 *             -------------------------------------------------------------------------------
 ...
 *                 }
 *              } 
 *              멤버변수 , 멤버 메소드, 생성자 ==> 클래스 소속 (static은 X)  
 *              -----------------------------------------------
 ...
 *    -------
 *    자바 상속의 특징
 *    = 재사용 목적으로 사용한다(기존에 만들어진 클래스를 확장해서 사용)
 ...
 *      class B extends A
 *      {
 *         void display(){} //수정이 필요한것만 수정 가능
 *      }                                        
 *    
 *    = 기존에 만들어진 클래스를 확장해서 새로운 클래스를 만들어 사용
 *      예)
 *          class 게시판
 *          {
 *             글쓰기
...
// Super로부터 상속을 받는다 => 예외조건) 생성자, 초기화 블록, static(공유는 가능하나 상속 X)

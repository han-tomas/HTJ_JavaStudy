 ...
 *                              운영체제마다 다르다.
 *         Java => 소스 작성 ==> 컴파일
 *                             .class 운영체제마다 동일한 파일 생성. = 호환성이 좋다.
 *         ** 자바 실행
 ...
 *         = 비교적 배우기 쉽다
 *         C/C++에서 파생 => 포인터(메모리 주소), 구조체, 다중상속, 지원하는 API 미약
 *          => 자바언어 => 강력한 API를 가지고 있다.(필요한 기능이 이미 제작)
 *             => 조립식 (자바 공부 => 직접 구현)
 *         = 자동 메모리 관리 (가비지 컬렉션) 
 *         
 *         = 네크워크 / 분산 / 멀티쓰레드 기능을 가지고있다.
 *    2. 소스 코딩 방법
 ...
 *    -----
 *     정수 저장 방법 
 *       1byte => byte byte a = 10; // 네트워크 전송, 파일 읽기... => 메모리 절약 (메모리가 부족하지 않은 현대에는 사용 거의x)
 *       2byte => short // C언어와 호환
 *       4byte => int // 평상시에 사용하는 모든 정수 (default) => 속도, 최적화
 ...
 *     문자 저장 방법
 *       2byte => char
 *         => 1byte(싱글바이트 = ASCII) / 2byte(멀티바이트 => Unicode)
 *         *** 한글이나 중국, 독일 , 일본 .. 한글당 바이트가 2byte
 *         byte => bit(8) => 256문자
 *         2byte => bit(16) => 65535문자
 *         UTF-8
 *         각 문자마다 번호 (ASCII code)
 *     논리 저장 방법
 ...
 *        
 *     8byte => 경 단위 ==> long  ex) long a = 100000000000L;
 *     
 *     변수
 *     데이터형 변수명 = 값;
 *     -----------------
 *     
 *     값을 부여할 때, 크기에 맞게 지정한다.
 *     
 *     int a = 10.5 (x) ==> 오류
 *     ---     ----
 *     4byte   8byte
...
//		double d = 123456.78;
//		int e = (int)((d-c)*100);
//		System.out.println(e); //부동소수점
		
		//***
//		byte byte_value = 127; // -128~127
		// 127을 byte_value라는 공간에 첨부해라 => 뒤에서부터(오른쪽에서 왼족으로) 계산
...
//		boolean boolean_value = true;
		...
		// 상수 => 데이터형 변수명 = 값  ex) final int b = 20;
//		int kor = 80; //변수
//		kor = 90; // 변경
//		kor = 100; // 변경2
		
//		final int SCORE = 70; //상수 => 상수명은 대문자
		//score = 100; // 변경불가(오류)

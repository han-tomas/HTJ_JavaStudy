 ...
 *                  = 프로그램에 맞게 구현해서 사용
 *                    예)
 *                       void getConnection(); => 구현이 안된 이유 : 오라클/ MS-SQL/MYSQL/MariaDB ...=> 구현법이 다 달라서.
 *                       => 데이터베이스 연결                        
 *                  
 ...
 *                  묻고답하기 : (목록출력, 찾기, 글쓰기, 내용보기, 수정하기 , 삭제하기), + 답볍하기 
 ...
 *                                          ------- 구현이 안된 메소드
 *                      abstract void find(); => abstract가 붙으면 구현부{}가 필요없음 ==> 추상 메소드
 *                      board content(){}
 *                      void write(){}      
 *                      
 *           추상클래스의 특징  
 ...
 *              abstract class A
 *              => A a = new A() => 메모리 할당 X
 *           3) 상속을 내려서 구현된 클래스를 이용해 메모리 할당
 *              abstract class A
 *              class B extends A
 *              {
 *                 --- 구현이 안된 메소드를 구현한다. 
 *              }                  
 *              => A a = new B(); // 묵시적 형변환 => 메모리 할당 가능
 *                                 (double d = 10; 자동형변환)
 *              => A a = new B();
 *                 B b = (B)a;  // 명시적 형변환
 ...
 *                    => 공통적인 기능     
 *           
 *           5) 선언된 메소드가 있으면 => 상속시 반드시 구현해야 된다(강제성)                                    
 */
...
class 원 extends 도형
{
	...
	public void color() {  // 필요에 따라 오버라이딩

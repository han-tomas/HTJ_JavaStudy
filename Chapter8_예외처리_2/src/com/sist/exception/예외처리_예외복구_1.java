 ...
 *    ----- 사전에 에러를 방지하는 프로그램
 *          비정상 종료를 방지하고 정상 종료 가능하게 만드는 목적
 *                    계층구조 (상속도)
 *                    -----
 *                    
 *                    Object : 모드 클래스의 상위 클래스(모든 클래스는 Object상속을 받는다
 *                       |
 ...
 *               => Wrapper : 기본형을 사용하게 쉽게 클래스형으로 제작 
 *                            booldean => Boolean
 *                            int => Integer
 *                            double => Double 
 *                            Intege.parseInt(" 10"); => 윈도우/Web(문자열만 사용)
 *                            웹에서 전송된 데이터를 받는 경우
 *                            -----------------------
		...
		// TODO Auto-generated method stub
			...
			for(int i=1;i<=10;i++)
			{
				try
				{
					...
					System.out.println("0으로 나눌 수 없다."); //for문 안에 try ~ catch문이 있을경우 for문이 끝날때까지 제외처리
				}                                         //for문 밖에 있을 경우 예외문장이 실행될때 바로 종료

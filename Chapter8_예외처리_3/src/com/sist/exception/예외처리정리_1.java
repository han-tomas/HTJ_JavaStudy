 ...
 *       }catch(예외처리 종류)
 *       {
 *          try에 있는 소스를 실행하는 과정에서 에러 발생시 처리
 *       }
 *       finally
 *       {
 *          생략이 가능 : 무조건 수행한다. (try~catch 관계없이)
 *          => 서버 닫기, 오라클 닫기, 파일 닫기 ...
 *       }
 ...
 *                            ------------------
 *                            Exception/ Throwable
 *       *** catch 절은 순서가 존재한다 => 상위 클래스는 아래에 배치한다
 *       *** try절이 정상 수행을 하면 catch절은 수행하지 않는다
 *       *** try절에서 에러 발생시에는 에러가 난 곳 부터는 수행하지 않는다
 ...
 *          
 *       }    
 *       ==> catch는 위에서 한개씩 검색한 후에 해당 예외 처리 클래스를 찾아 준다

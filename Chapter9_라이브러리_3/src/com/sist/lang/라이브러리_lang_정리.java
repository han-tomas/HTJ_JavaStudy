 ...
 *              => 라이브러리에 제공하는 메소드
 *                 --------------------
 *                 리턴형/매개변수 ==> 대부분이 Object
 *                 => 형변환이 필요 => 제네릭스 (리턴형/매개변수 변경)
 *              => 기능
 *                 1) clone : 복제 (새로운 메모리를 만든다.)
 *                 2) toString : 객체를 문자열화
 *                 3) finalize : 소멸자
 *                 4) equals : 비교(default=>주소값 ==> 실제 값을 비교할때는 오버라이딩 필요)
 *                  
 *     String : 문자열 저장 클래스
 *              기능
 *              length()
...
public class 라이브러리_lang_정리 {
	...
	public static void main(String[] args) {
		...
		list.add("aaa");
		list.add(1);
		String a = (String)list.get(0); //리턴형이 Object이기때문에 형변환(String)이 필요
		int b = (Integer) list.get(1); //리턴형이 Object이기때문에 형변환(int)이 필요
	}

...
class Sawon implements Cloneable{
	...
	{
		// 지역 변수가 우선순위이기 때문에, this를 이용해 멤버변수와 지역변수를 구분해 주어야 한다.
		// this는 자신의 객체(멤버변수) => static 변수, 모든 클래스는 this를 가지고있다.
		this.sabun = sabun;
		this.name = name;
		/*
		 *    오버로딩
		 ...
		 *    // 생성자는 무조건 한개 이상 있다.
		 *    // 생성자를 사용하지 않으면 자동으로 기본 생성자Sawon()를 추가해준다.
		 *    // 시작과 동시에 필요한 내용을 서술
...
public class 라이브러리_Object_1 {
	...
	public static void main(String[] args) throws CloneNotSupportedException {
		...
		s1.setSabun(2);
		s1.setName("심청이");
		// 주소를 이용할 때 같은 주소인 경우 => 다른객체의 내용도 변경된다.

 ...
 *    boolean endsWith(String s) //리턴형 boolean
 *    String concat(String s) //리턴형 String
 *    ----------------------------------------------
 *    
 *    String 메소드 (중요)
 *    charAt()
...
public class 라이브러리_String_5 {
	...
	public static void main(String[] args) {
		...
		if(s2.endsWith("!!"))
			System.out.println("!!로 끝났다.");
		else
			System.out.println("!!로 끝나지 않았다.");*/
		...
		System.out.println(String.valueOf(10)+String.valueOf(30)); //1030
		// "10" + "30" ==> "1030"
		...
		ss="서울특별시 응산구 소월로2길 27 지번 서울시 용산구 후암동 445-8";
		...
		// 사용처 : URL 주소 제어, 주소 제어, 문자열이 긴 경우 "..."로 표현
		...
		ss="1인 사시미"+" 22,000원"+"항정살 구이" +" 22,000원"+"토마토스튜"+" 22,000원"+"브루스게타"+" 18,000원"+"닭다리숯불구이"+" 18,000원";
		...
		String[] colors = ss.split("\\|"); // | 정규식에서 사용하는 기호 "\\|" 사용
		                                 // | , ?, . , + , * , ^
		...
		// 오라클에 저장시에 &는 오라클에서 Scanner이므로 문제 발생 
		res = ss.replace("&", "@");
		System.out.println(res);

...
public class StudentSystem {
	...
	private Student[] std = new Student[3]; // 배열은 메모리 크기를 생성한것, 메모리 값(주소값)을 설정한 것은 아님.
	                                        //                        저장될 위치가 지정이 안됨.
	 ...
	 *      ------- 제어문, 외부 라이브러리 호출 불가능 {}                               
	 */
	public void init() // 초기화 해주는 메소드
	{
		Scanner scan = new Scanner(System.in);
		for(int i=0;i<std.length;i++)
		{
			std[i]=new Student(); // 배열은 모든 저장공간이 null(메모리 연결이 필요함)
//			       -------------- 메모리 연결하는 부분 
	...
	//1-1. 출력
//	void display() //default ==> com.sist.main4안에서만 접근이 가능
	public void display() //     com.sist.main5에서 접근하려면 public 을 붙여야한다.
	{             
		for(Student s:std)
		{
